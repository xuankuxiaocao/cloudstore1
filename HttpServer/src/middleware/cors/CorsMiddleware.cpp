...
namespace http
{
    namespace middleware
    {
        ...
        {
            return config_.allowedOrigins.empty() || // 为空，不做任何限制
                   std::find(config_.allowedOrigins.begin(),
                             config_.allowedOrigins.end(), "*") != config_.allowedOrigins.end() || // 允许所有其他源访问
                   std::find(config_.allowedOrigins.begin(),
                             config_.allowedOrigins.end(), origin) != config_.allowedOrigins.end(); // 查找指定的源origin是否在vector数组中.
        }
