...
namespace http
{
    ...
    // 这是从问号后面分割参数
    //假设收到URL：/api/user?name=John&age=25
    //会预先根据?裁剪，所以传到该函数里的只有name=John&age=25
    void HttpRequest::setQueryParameters(const char *start, const char *end)
    {
        ...
        // 按 & 分割多个参数
        //(pos = argumentStr.find('&', prev)) 从prev开始寻找&字符，未找到则自动赋值为npos
        //name=John&age=25 在这个循环里被切割，因为是以&为切割符的，所以while只会循环一次，分成两个：1.name=John 2.age=25
        //所以填入queryParameters_[name]=John,queryParameters_[age]=25
